    ...
    for (path, dirs, files) in os.walk(base_path):
        if files:
            if 'truth.txt' in files:
                ...
                instance, feature_list = read_docs_to_data(path, known_files, 'unknown.txt', language)
                #instance.append(truth_dict[problem_name])
                data.append(instance)
    return_features(feature_list)
...
def read_docs_to_data(path, files, file_unknown, language):
    ...
    for file in files:
        document = codecs.open(os.path.join(path, file), encoding='utf-8-sig', errors='replace').read()
