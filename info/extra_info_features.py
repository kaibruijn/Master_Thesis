...
def main():
    features = ""
    with open("../features.txt") as text:
        for line in text:
            features+=line
    features = features[:-1]
    ...
    for i in triplist:
        if i[2] == "R" and i[6] != "-" and i[7] == i[6] and i[4] != "-" and i[5] != "-" and abs(int(i[4])-int(i[5])) < 10:
            print(i)
            Rlist.append(float(i[1]))
        if i[2] == "S" and i[6] != "-" and i[7] == i[6] and i[4] != "-" and i[5] != "-" and abs(int(i[4])-int(i[5])) < 10:
            #print(i)
            Slist.append(float(i[1]))
        if i[2] == "T":
            #print(i)
            Tlist.append(float(i[1]))
        if i[2] == "U" and i[6] != "-" and i[7] == i[6]:
            #print(i)
    ...
    text_file = open("analysis.txt", "a")
    text_file.write(features+","+str(sum(Tlist)/len(Tlist))+","+str(sum(Slist)/len(Slist))+","+str(sum(Rlist)/len(Rlist))+","+str(sum(Ulist)/len(Ulist))+"\n")
    text_file.close()

    print(features+","+str(sum(Tlist)/len(Tlist))+","+str(sum(Slist)/len(Slist))+","+str(sum(Rlist)/len(Rlist))+","+str(sum(Ulist)/len(Ulist)))
    ...
    Rlist = []
    Ulist = []
    with open("analysis.txt") as txt:
        for line in txt:
            featurelist.append(line.split(",")[0])
            Tlist.append(float(line.split(",")[1]))
            Slist.append(float(line.split(",")[2]))
            Rlist.append(float(line.split(",")[3]))
            Ulist.append(float(line.split(",")[4]))

    data = pd.DataFrame(list(zip(featurelist, Tlist, Slist, Rlist, Ulist)), columns=["features", "T", "S", "R", "U"])

    data.to_excel("features.xlsx", index=False) 
